function [p_new,t_new,I,B] = MeshDirSquare(param)

%get parameters
L=param.L; h=param.h; H=param.H; height=param.height; L1=param.L1;
...
v1 = [0,-H; 0,-L1; L,-L1; L,-H];  
g1 = [3;4;v1(:,1);v1(:,2)]; % [3,4,x1,x2,x3,x4,y1,y2,y3,y4]'
v2 = [0,-L1; 0,L1; L,L1; L,-L1]; 
g2 = [3;4;v2(:,1);v2(:,2)]; % [3,4,x1,x2,x3,x4,y1,y2,y3,y4]'
v3 = [0,L1; 0,H; L,H; L,L1]; 
g3 = [3;4;v3(:,1);v3(:,2)]; % [3,4,x1,x2,x3,x4,y1,y2,y3,y4]'
v4 = [L/3,-height; L/3,height; 2*L/3,height; 2*L/3,-height]; 
g4 = [3;4;v4(:,1);v4(:,2)]; % [3,4,x1,x2,x3,x4,y1,y2,y3,y4]'

geom_matrix = [g1 g2 g3 g4];
set_formula = 'g1+(g2-g4)+g3';    

% Add the geometry to the PDE model
geometryFromEdges(model, decsg(geom_matrix, set_formula, char('g1', 'g2', 'g3', 'g4')'));
...
fd = @(p) max(-min(min(min(H+p(:,2),H-p(:,2)),p(:,1)),L-p(:,1)),...
    min(min(min(height+p(:,2),height-p(:,2)),-L/3+p(:,1)),2*L/3-p(:,1))); %function used to get boundary elements
