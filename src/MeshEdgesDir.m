function [LI,LU,LD,LR,LDir] = MeshEdgesDir(mesh,param)
...
for j=1:size(LB,1)
    ...
    elseif abs(p(LB(j,1),2)-H)<=toll && abs(p(LB(j,2),2)-H)<=toll %upper
        nu=nu+1;
        LU(nu,:)=LB(j,:);
    elseif abs(p(LB(j,1),2)+H)<=toll && abs(p(LB(j,2),2)+H)<=toll %lower
        nd=nd+1;
        LD(nd,:)=LB(j,:);
    else %otherwise, it is a Dirichlet boundary edge
        ndir=ndir+1;
        LDir(ndir,:)=LB(j,:);
    end
end
