...
nd_raff=15; %number of directions for the refined solution


%-----------------------------------
%Mesh definition
%-----------------------------------
domain = 'layers_rectangle'; %select domain
[mesh,param] = GenerateMesh(param,domain); %generate mesh 

disp(['DtN-TDG p-convergence test on the domain with multiple materials: k=',...
...
%-----------------------------------
disp(['Computing refined solution: p=', num2str(nd_raff), ', ',...
...
for nd=ni:nf %cycle on number of PW directions
    ...
    disp(['Linear system assembly: p=', num2str(param.nd),...
        ', ', num2str(param.nd.*size(mesh.t,1)), ' basis functions' ])
...
%-----------------------------------
disp('Plotting numerical solution and error')
