function PlotSolutionExact(mesh,param,u,phi,uex)
...
for T=1:size(t1,1)
    ...
    if T==1
        ...
        minColorLimit2 = min(min((real(diff))));
        maxColorLimit2 = max(max((abs(diff))));
    else
        ...
        minColorLimit2 = min(min((real(diff))),minColorLimit2);
        maxColorLimit2 = max(max((abs(diff))),maxColorLimit2);
    end
    ...
    subplot(2,2,3)
    trisurf(t,p(:,1),p(:,2),(real(diff))); shading flat; 
    clim([minColorLimit2,maxColorLimit2]); axis equal; hold on
    view(2)
    set(gca,'fontsize',12)

    subplot(2,2,4)
    trisurf(t,p(:,1),p(:,2),(abs(diff))); shading flat; 
    clim([minColorLimit2,maxColorLimit2]); colorbar; axis equal; hold on
    view(2)
    set(gca,'fontsize',12)
