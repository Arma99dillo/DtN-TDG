function [val1,val2,val3]=DtNInt_m(p,dl,dj,nd,k,l1,l2,L1,L2,j,l,coeff,M,alpha_n,beta_n,H,S)

toll=1e-6;

x1=p(l1(1),1); x2=p(l1(2),1); %endpoints phi_l
x1t=p(l2(1),1); x2t=p(l2(2),1); %endpoints phi_j
...
for n=-M:M
    if abs(alpha_n(v)-k*dj(1)) <= toll
        val1=val1+(1i*coeffl(v)*beta_n(v)).*exp(1i*k*dj(2)*H).*abs(x2t-x1t);
    else
    ...
    if abs(alpha_n(v)-k*dl(1)) <= toll
        val2=val2+(1i*coeffj(v)*beta_n(v))'.*exp(-1i*k*dl(2)*H).*abs(x2-x1);
    else
